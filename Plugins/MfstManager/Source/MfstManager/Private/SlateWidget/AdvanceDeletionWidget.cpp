...
{
	FString DisplayAssetName = AssetDataToDisplay->PackageName.ToString();
	const int32 Index=DisplayAssetName.Find(TEXT("/"),ESearchCase::IgnoreCase,ESearchDir::FromEnd);
	DisplayAssetName = DisplayAssetName.RightChop(Index+1);
	...
	TSharedRef<STableRow<TSharedPtr<FAssetData>>> ListViewRowWidget =
		...
		[
			...
			+SHorizontalBox::Slot()
			[
				ConstructTextRowWidget(DisplayAssetPath)
			]
			//4.button
			+SHorizontalBox::Slot()
			.HAlign(HAlign_Right)
...
{
	FMfstManagerModule::SyncCBToClickedAssetForAssetList(ClickedData->PackageName.ToString());
}
...
void SAdvanceDeletionTab::OnComboSelectionChanged(TSharedPtr<FString> SelectedOption, ESelectInfo::Type InSelectInfo)
{
	...
	else if (*SelectedOption.Get() == TEXT("List Same Name Assets"))
	{
		FMfstManagerModule::ListSameNameAssetsForAssetList(StoredAssetData,DisplayedAssetsData);
		RefreshAssetListView();
	}
}
